...
namespace UniRx
{
    ...
    public class ReactiveProperty<T> : IReactiveProperty<T>, IDisposable, IOptimizedObservable<T>, IObserverLinkedList<T>
    {
        ...
        public IDisposable Subscribe(Action<T> onNext, bool raiseLatestValueOnSubscribe)
        {
            return Subscribe(Observer.CreateSubscribeObserver(onNext, Stubs.Throw, Stubs.Nop), raiseLatestValueOnSubscribe);
        }

        public IDisposable Subscribe(IObserver<T> observer, bool raiseLatestValueOnSubscribe)
        {
            if (isDisposed)
            {
                observer.OnCompleted();
