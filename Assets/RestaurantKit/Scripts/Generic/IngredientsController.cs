...
public class IngredientsController : MonoBehaviour {
	...
	void createIngredient (){
		if(canCreate && !MainGameController.Instance.gameIsFinished && !MainGameController.Instance.deliveryQueueIsFull) {
			canCreate = false;
			itemIsInHand = true;
			...
			prod.transform.localScale = new Vector3(0.17f, 0.01f, 0.135f); //hardcoded scale value. change with caution!
			playSfx(itemPick);
	...
	void createRawIngredient (){
		if(canCreate && !MainGameController.Instance.gameIsFinished) {
			canCreate = false;
			itemIsInHand = true;
			GameObject prod = Instantiate(ingredientsArray[factoryID - 1], transform.position + new Vector3(0,0, Zoffset), Quaternion.Euler(90, 180, 0)) as GameObject;
